   ...
   if(!empty($cardno))
{
 ...
 //echo $num;
  ...
  {
    echo "<script>if(confirm('invalid coupon number!!!!')){document.location.href=\"payment.php?empid= $eid &&sid=$s&&scid=$sc&&bid=$bid\"}else{document.location.href='payment.php?empid= $eid &&sid=$s&&scid=$sc&&bid=$bid'};</script>";
  }
 ...
 while($fesx=mysqli_fetch_array($esx)){
  echo $acc=$fesx['acc_num'];
 echo $cvv1=$fesx['cvv'];
 }
   if($acc!=$cno && $cvv!=$cvv1)
   {
     //echo "hi";
  // return;
    echo "<script>if(confirm('invalid Account number or CVV!!!!')){document.location.href=\"payment.php?empid= $eid &&sid=$s&&scid=$sc&&bid=$bid\"}else{document.location.href='payment.php?empid= $eid &&sid=$s&&scid=$sc&&bid=$bid'};</script>";
   }
   ...
   else
{
 ...
 //return;
  ...
  if($price>$r1)
  {
    echo "<script>if(confirm('no balance!!!!')){document.location.href='payment.php?empid= $eid &&sid=$s&&scid=$sc&&bid=$bid'}else{document.location.href='payment.php?empid= $eid &&sid=$s&&scid=$sc&&bid=$bid'};</script>";
...
$message1 ="<main>
 ...
 you are booked on : $cdate<br>
  ...
  window.location.href='satff.php'</script>  
  <?php
   } 
   else
   {
