 ...
 // $stat = $_POST['stat'];
  ...
  $_SESSION['ln']=$ln;
   ...
   $sql = "SELECT * FROM `tb_login` WHERE Username='$email'";
    ...
    echo"<script> alert('This Email is already exist!!! Please choose another one');window.location ='../empreg.php';</script>";
    }
    else if(mysqli_num_rows($res1)>0 ){
     echo"<script> alert('This Mobile is already exist!!! Please choose another one');window.location ='../empreg.php';</script>";
 
    }
    if(mysqli_num_rows($res2)>0 ){
        echo"<script> alert('This Aadhar Number is already exist!!! Please choose another one');window.location ='../empreg.php';</script>";
        }
        if(mysqli_num_rows($res3)>0 ){
            echo"<script> alert('This Username  is already exist!!! Please choose another one');window.location ='../empreg.php';</script>";
            }
 }
   else{
...
$r=$rowp['cat_id'];
   $quer=("SELECT * FROM tb_service where cat_id='$r'");
...
$sql1=("select * from tb_role where role='employee'");
    ...
    $query2 = "INSERT INTO `tb_place`(`Place`, `Did`) VALUES ('$pla', '$distr')";
   ...
   if($result=1){
     ...
     $query = "INSERT INTO `tb_regsuser`(`Log_id`,  `first_name`,  `last_name`, `hname`, `Photo`, `Pin`, `Gender`, `DOB`,`Pid`, `Mobile`, `Aadhar`,`Aadhar_pic`) 
   VALUES ('$logid', '$fn', '$ln', '$hname', '$path', '$pin', '$gen', '$dob','$pid', '$mob', '$aadhar','$paths')";
	 

     $resultm = mysqli_query($con, $query);
     //echo  $query;
     $query3 = "INSERT INTO `tb_empserv`(`Log_id`, `cat_id`) VALUES ('$logid', '$cat')";
   $result42 = mysqli_query($con, $query3);
