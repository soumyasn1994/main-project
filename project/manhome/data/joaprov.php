...
$eid=$_GET['eid'];
 $bid=$_GET['bid'];
		echo $sql="select * from tb_bookings where Log_id='$id'";
...
{
            //echo $sql="update tb_bookings set status = '1' where  Log_id='$id'";
            $sq=mysqli_query($con,"update tb_bookings set status = '1' where  Log_id='$id'");

}
else
{
            echo $sql="update tb_bookings set status = '2' where Log_id='$id'";
...
$sql1="SELECT * FROM `wallet` WHERE `Log_id`='1'";
  ...
  $sql4="SELECT * FROM `wallet` WHERE `Log_id`='$eid'";
  $result4=mysqli_query($con,$sql4);
  $row4=mysqli_fetch_array($result4);
   $r1=$row4['balance'];
  
  $sql8="SELECT * FROM `tb_bank` WHERE `Log_id`='$id'";
  ...
  if($amt>$r1)
  {
    echo "<script>if(confirm('no balance!!!!')){document.location.href='../staff.php'}else{document.location.href='../staff.php'};</script>";

  }
  else
  {
  $sql6="UPDATE `tb_bank` SET `balance`='$user' WHERE `Log_id`='$id'";
...
header("Location:../jobaprov.php");
       ...
       // return;
            ...
            //return;
				header("refresh:0; url=http://localhost/manhome/jobaprov.php");
